...
**환경**이 정확하게 무엇이냐면 값들이 저장되어있는 저장소임
...
asdf=100
```
이런식으로 띄어쓰기 없어도 상관은 없는데 띄어쓰기 해주는게 국룰임 (안지키면 죽음)

### 변수 이름의 규칙
변수 이름은 영어 대소문자와 숫자와 _만 사용 가능하고 숫자로 시작할수없음
```luau
aSdF_ = 30
_ddd12323 = 100
123good = 111
```
이 코드에선 변수 123good가 1로 시작하기 때문에 에러가 남
...
not A에서 A가 (nil 또는 boolean)이면 true 아니면 false를 반환하는 문법이다
```luau
